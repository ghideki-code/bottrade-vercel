...
async function load(symbol:string, interval:string):Promise<Candle[]> {
  const r=await fetch(`https://fapi.binance.com/fapi/v1/klines?symbol=${encodeURIComponent(symbol)}&interval=${interval}&limit=120`);
...
export async function GET(req:Request){
  try{
    ...
    return new Response(JSON.stringify({results,limit:symbols.length,source:'Binance USDⓈ-M Futures • Batch Setup Engine'}),{headers:{'content-type':'application/json','cache-control':'s-maxage=10, stale-while-revalidate=30'}});
