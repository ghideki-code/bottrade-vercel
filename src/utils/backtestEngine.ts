...
export function backtestSetup(c15:Candle[],c1h:Candle[],c4h:Candle[],minScore=65,maxBars=32):BacktestResult{
 ...
 for(let i=60;i<c15.length-maxBars-1;i++){if(i<nextAvailable)continue;const t=c15[i].timestamp,h1=c1h.filter(x=>x.timestamp<=t).slice(-200),h4=c4h.filter(x=>x.timestamp<=t).slice(-200);if(h1.length<50||h4.length<50)continue;const setup=detectScalpSetup(c15.slice(0,i),h1,h4);if(setup.side==='WAIT'||setup.score<minScore)continue;
  ...
  if(result==='LOSS')realizedR=-1;else if(result==='TIMEOUT'){const remaining=1-stage/3;const move=setup.side==='LONG'?(exit-entry)/risk:(entry-exit)/risk;realizedR+=remaining*move/Math.max(1,remaining);}
