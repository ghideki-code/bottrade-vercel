...
export interface ConfluenceAnalysis {
  ...
  wyckoff:{phase:WyckoffPhase;score:number;event:string;effortResult:string}; smc:ReturnType<typeof analyzeSMC>;
  divergence:'BULLISH'|'BEARISH'|'NONE'; volumeRatio:number; structure:'BULLISH'|'BEARISH'|'MIXED';
  squeeze:{active:boolean;compression:number;momentum:'BULLISH'|'BEARISH'|'NEUTRAL'}; reasons:string[];
}
...
export function analyzeConfluence(candles:Candle[]):ConfluenceAnalysis{
  ...
  if(last.close<oldPrice&&r>oldR+3)divergence='BULLISH';else if(last.close>oldPrice&&r<oldR-3)divergence='BEARISH';
  ...
  const avgRange=c.slice(-20,-1).reduce((a,x)=>a+(x.high-x.low),0)/19,recentRange=c.slice(-5).reduce((a,x)=>a+(x.high-x.low),0)/5;
  const compression=avgRange?Math.max(0,Math.min(100,(1-recentRange/avgRange)*100)):0,squeezeActive=compression>=25;
  const squeezeMomentum:ConfluenceAnalysis['squeeze']['momentum']=last.close>e21?'BULLISH':last.close<e21?'BEARISH':'NEUTRAL';
  const components={smc:smc.score,structure:structure==='BULLISH'?100:structure==='BEARISH'?0:50,wyckoff:phase==='ACCUMULATION'||phase==='MARKUP'?100:phase==='DISTRIBUTION'||phase==='MARKDOWN'?0:50,divergence:divergence==='BULLISH'?100:divergence==='BEARISH'?0:50,volume:Math.max(0,Math.min(100,50+(volumeRatio-1)*50)),squeeze:squeezeActive?(squeezeMomentum==='BULLISH'?80:squeezeMomentum==='BEARISH'?20:50):50};
  const score=Math.round(components.smc*.35+components.structure*.20+components.wyckoff*.15+components.divergence*.10+components.volume*.10+components.squeeze*.10),bias=score>=62?'BULLISH':score<=38?'BEARISH':'NEUTRAL';
  const reasons:string[]=[];if(smc.reasons.length)reasons.push(...smc.reasons.slice(0,2));if(structure!=='MIXED')reasons.push(`Estrutura ${structure.toLowerCase()} (EMA 9/21/50)`);if(phase!=='RANGE')reasons.push(`Wyckoff ${phase.toLowerCase()}`);if(divergence!=='NONE')reasons.push(`Divergência ${divergence.toLowerCase()}`);if(volumeRatio>1.5)reasons.push(`Volume elevado (${volumeRatio.toFixed(2)}x)`);if(squeezeActive)reasons.push(`Compressão ${compression.toFixed(0)}%`);
